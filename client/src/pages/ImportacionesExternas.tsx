  ...
  return <div className="space-y-6 p-4 sm:p-6 lg:p-8">
    ...
    <div className="grid gap-4 xl:grid-cols-2">
      ...
      <Card className="border-sky-200 shadow-sm">
        ...
        <CardContent className="space-y-4"><div className="rounded-lg border bg-slate-50 p-4 text-sm"><p className="font-medium text-slate-900">Cómo preparar la conexión</p><ol className="mt-2 list-decimal space-y-1 pl-5 text-muted-foreground"><li>En Cloudbeds, abre <strong>Integraciones de API y Credenciales</strong> y pulsa <strong>Nuevas credenciales</strong>.</li><li>Crea una credencial OAuth y conserva el <strong>ID de cliente</strong> y el <strong>secreto de cliente</strong>.</li><li>Cuando los tengas, introdúcelos mediante la configuración segura del servidor; nunca se guardarán en esta pantalla ni en la base de datos.</li></ol></div><div className="rounded-md border border-sky-100 bg-sky-50 px-3 py-2 text-xs text-sky-950"><strong>Estado:</strong> la conexión se activará solo cuando se hayan proporcionado ambas credenciales y se configure la autorización de la propiedad.</div><Button variant="outline" className="w-full" asChild><a href="https://developers.cloudbeds.com/reference/about-pms-api" target="_blank" rel="noreferrer">Ver documentación de Cloudbeds <ExternalLink className="ml-2 h-4 w-4" /></a></Button></CardContent>
      </Card>
    </div>
    ...
    <div className="grid gap-4 md:grid-cols-3">
      ...
      <Card><CardContent className="pt-6"><p className="text-sm text-muted-foreground">Ventas importadas del período</p><p className="mt-2 text-3xl font-bold">{money((overview.data?.dailyCash || []).filter((record: { businessDate: string }) => record.businessDate >= dateFrom && record.businessDate <= dateTo).reduce((sum: number, item: { totalSales: string | number | null }) => sum + Number(item.totalSales || 0), 0))}</p></CardContent></Card>
    </div>
