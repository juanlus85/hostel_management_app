...
import { CheckCircle2, Copy, Loader2, MapPin, ShieldCheck, Wifi, KeyRound, Building2, Phone, Mail, Info } from "lucide-react";
...
export default function CheckinOnlinePublico() {
  ...
  if (submitted) return <ArrivalConfirmation lang={lang} guestName={form.firstName} data={submitted} />;
...
function ArrivalConfirmation({ lang, guestName, data }: { lang: Language; guestName: string; data: OnlineCompletion }) {
  const t = (es: string, en: string) => lang === "es" ? es : en;
  ...
  const intro = lang === "es" ? data.arrivalIntroEs : data.arrivalIntroEn;
  const keyInstructions = lang === "es" ? data.keyInstructionsEs : data.keyInstructionsEn;
  const commonAreas = lang === "es" ? data.commonAreasEs : data.commonAreasEn;
  const houseRules = lang === "es" ? data.houseRulesEs : data.houseRulesEn;
