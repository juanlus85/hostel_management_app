...
export default function CheckinOnlinePublico() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      const result = await complete.mutateAsync({ ...form, token, signature: signatureCanvasRef.current?.toDataURL("image/png") || "", acceptedTerms: true, acceptedPrivacy: true });
      setSubmitted(result);
  ...
  if (submitted) return <ArrivalConfirmation lang={lang} guestName={form.firstName} guestEmail={form.email} data={submitted} welcomeTemplate={welcomeTemplate} />;
...
function ArrivalConfirmation({ lang, guestName, guestEmail, data, welcomeTemplate }: { lang: Language; guestName: string; guestEmail: string; data: OnlineCompletion; welcomeTemplate?: string }) {
  const t = (es: string, en: string) => lang === "es" ? es : en;
  ...
  const welcomeMessage = renderArrivalTemplate(welcomeTemplate, templateContext);
  ...
  const shareText = [data.hostelName, "", welcomeMessage || intro || defaultIntro, "", `${t("Código de entrada", "Entrance code")}: ${data.entranceCode || "—"}`, `${t("Código de habitación", "Room code")}: ${data.roomCode || "—"}`, `${t("Habitación", "Room")} ${data.roomNumber}${data.roomType ? ` · ${data.roomType}` : ""}${floor ? ` · ${floor}` : ""}`, data.hostelAddress || "", data.arrivalMapUrl || "", data.wifiPassword ? `Wi‑Fi: ${data.wifiPassword}` : "", commonAreas || "", houseRules || "", [data.hostelPhone, data.hostelEmail].filter(Boolean).join(" · ")].filter(Boolean).join("\n");
