...
function firstGuest(detail: Record<string, unknown>) {
  ...
  return asRecord(detail.guest ?? detail.primaryGuest ?? detail.primary_guest);
}

function firstRoom(detail: Record<string, unknown>) {
  const rooms = detail.rooms ?? detail.accommodations;
...
export function normalizeUpcomingReservation(assignment: CloudbedsReservationAssignment, detail: CloudbedsReservationDetail, fallbackDate: string): ImportedUpcomingReservation | null {
  ...
  const checkInDate = firstText(detailRecord.checkInDate, detailRecord.check_in_date, detailRecord.arrivalDate, detailRecord.arrival_date, assignmentRecord.checkInDate, assignmentRecord.check_in_date, assignmentRecord.date) ?? fallbackDate;
  if (!sourceReservationId || !/^\d{4}-\d{2}-\d{2}$/.test(checkInDate)) return null;
  return {
    sourceReservationId,
    reservationCode: firstText(detailRecord.reservationCode, detailRecord.reservation_code, detailRecord.confirmationCode, detailRecord.confirmation_code, assignmentRecord.reservationCode),
    guestName: firstText(guest.name, guest.fullName, guest.full_name, detailRecord.guestName, detailRecord.guest_name, `${text(guest.firstName) ?? ""} ${text(guest.lastName) ?? ""}`.trim()),
    guestEmail: firstText(guest.email, guest.emailAddress, guest.email_address, detailRecord.guestEmail, detailRecord.guest_email),
    guestPhone: firstText(guest.phone, guest.phoneNumber, guest.phone_number, detailRecord.guestPhone, detailRecord.guest_phone),
    checkInDate,
    checkOutDate: firstText(detailRecord.checkOutDate, detailRecord.check_out_date, detailRecord.departureDate, detailRecord.departure_date, assignmentRecord.checkOutDate, assignmentRecord.check_out_date),
    roomType: firstText(room.roomTypeName, room.room_type_name, room.roomType, detailRecord.roomTypeName, detailRecord.room_type_name, assignmentRecord.roomTypeName, assignmentRecord.room_type_name),
    roomNumber: firstText(room.roomName, room.room_name, room.roomNumber, room.room_number, detailRecord.roomNumber, detailRecord.room_number, assignmentRecord.roomName, assignmentRecord.room_number),
...
export async function fetchCloudbedsUpcomingReservations(apiKey: string, propertyId: string, dates: string[]) {
  ...
  for (const date of dates) {
    ...
    for (const assignment of assignments) {
      ...
      const detail = asRecord(await getJson(detailUrl, apiKey));
