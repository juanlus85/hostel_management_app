...
describe("aggregateCloudbedsPaymentsByOperationalDay", () => {
  ...
  it("consulta transacciones con rango, API key y Property ID sin exponerlos", async () => {
    ...
    expect(JSON.parse(fetchMock.mock.calls[0][1].body)).toMatchObject({ filters: { and: [{ field: "transaction_datetime", operator: "greater_than_or_equal" }, { field: "transaction_datetime", operator: "less_than_or_equal" }] }, limit: 1100 });
  });
});
